...
void MeshTools::CalcWeight(Mesh& mesh)
{
	...
	for (auto heh : mesh.halfedges())
	{
		weight[heh] = 0.5 * (cot[heh] + cot[heh.opp()]);	
		//weight[heh] = 1;
	}
}
...
{
	CalcWeight(mesh);//计算边权
	std::map<int, int> compressedIndex;
	SpMat coeMat;
	std::vector<Eigen::VectorXd> bias;
	bias.resize(3);
	CalcCoeMat(mesh, deformedMesh, compressedIndex, bias, coeMat);//计算w_ij
	Eigen::SimplicialCholesky<SpMat> chol(coeMat);

	for (int i = 0; i < 5; i++)
	{
		CalcS(mesh, deformedMesh);//根据变形网格计算每点的S矩阵
		CalcR(mesh);//根据每点的S计算变换矩阵R
		std::vector<Eigen::VectorXd> biasCopy = bias;
		CalcBVec(mesh, compressedIndex, biasCopy);//计算方程的右端项量
		SolveUpdate(mesh, deformedMesh, compressedIndex, chol, biasCopy);//计算p'
	}
}
